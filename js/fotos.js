...
function imprime_(a){document.write(a);}
var photosize;
if(!photosize){photosize = 720;}

var columns;
if(!columns || isNaN(columns) || columns < 1) {columns = 5;}
...
function picasaweb(j){ //returns the list of all albums for the user
 ...
 imprime_("<div style='margin-left:3px' style='margin-bottom:10px' class='galeriaFotosTop' ><strong>"+titulo+"</strong></div>");
 imprime_("<table width='100%' border='0' cellspacing='0' cellpadding='0'><tr>");
 ...
 for(i=0;i<j.feed.entry.length;i++){
  ...
  imprime_("<td valign=top class='InicioGaleria'><a href='?id="+_GET['id']+"&albumid="+id_base+"'><img src='"+img_base+"?imgmax=160&crop=1'  height='160' width='160' hspace='8' /></a>");
  imprime_("<br><center><a style='font-size:11px' href='?id="+_GET['id']+"&albumid="+id_base+"'>"+ j.feed.entry[i].title.$t +" </a></center></td>");
  if (i % columns == columns-1) {
    imprime_("</tr><tr><td colspan='"+columns+"'>&nbsp;</td></tr> <tr>");
  }
 }
 imprime_("</tr></table>");
...
function albums(j){  //returns all photos in a specific album
 ...
 imprime_("<div style='margin-left:3px' class='galeriaFotosTop'><a href='" + window.location.protocol + "//" + window.location.hostname+window.location.pathname+"?id="+_GET['id']+"'>"+titulo+"</a> &gt; "+ j.feed.title.$t +"&nbsp;&nbsp;["+np+" Foto"+item_plural+"]</div><br>");
 imprime_("<table width='100%' border='0' cellspacing='0' cellpadding='0'><tr>");
 ...
 for(i=0;i<j.feed.entry.length;i++){
  ...
  imprime_("<td valign=top><a href='"+link_url+"' rel='lightbox-"+_GET['albumid']+"' ><img src='"+img_base+"?imgmax=160&crop=1'  height='160' width='160' hspace='8' /></a>");
  //imprime_("<p><center><span STYLE='font-size: 12px'>"+j.feed.entry[i].media$group.media$description.$t+"</span></center>");
  imprime_("</td>");
...
imprime_("<div class='galeriaFotosTop2' >&nbsp;<div style='margin-left:3px; float: left;' ><a style='font-size:12px' href='"+ window.location.protocol + "//" + window.location.hostname+window.location.pathname+"?id="+_GET['id']+"'>"+titulo+" </a> &gt; <a  href='" + album_base_path + "'>" + my_fixed_galleryname + "</a> &gt; <!--" + img_filename + "-->" + current_index_text + "</div>");
...
if (n1 == null) //we're at the last picture in the album; going forward takes us to the album index
 ...
 imprime_("<div style='float:right;' >");
 if (photo_array.length > 1) { imprime_("<a  href='"+prev+"' style='border:0;'>Anterior</a>&nbsp;&#9474;&nbsp;"); }
 imprime_("<a  href='"+album_base_path+"' style='border:0;'>Inicio</a>&nbsp;&#9474;&nbsp;");
 if (photo_array.length > 1) { imprime_("<a  href='"+next+"' >Siguiente</a>"); }
 imprime_("</div></div><br>");
 ...
 //imprime_("<center><a border=0 target=PICASA href='"+photo_link+"'><img id='picture' width="+display_width+" src='"+img_base+"?imgmax="+photosize+"'  /></a></center>");
 imprime_("<center><img id='picture' maxwidth='720' src='"+img_base+"?imgmax="+photosize+"'  /></center>");
 imprime_("<br><center><div style='margin-left:0px' style='font-size:12px' >"+j.entry.media$group.media$description.$t+"</div></center></p>");
...
if(_GET['photoid']&&_GET['albumid']){
 ...
 //imprime_('http://picasaweb.google.com/data/entry/base/user/'+username+'/albumid/'+_GET['albumid']+'/photoid/'+_GET['photoid']+'?alt=json&callback=photo&np='+_GET['np']+'&galleryname='+_GET['galleryname']);
...
//imprime_('<script type="text/javascript" src="http://picasaweb.google.com/data/entry/base/user/'+username+'/albumid/'+_GET['albumid']+'/photoid/'+_GET['photoid']+'?alt=json&callback=photo&np='+_GET['np']+'&galleryname='+_GET['galleryname']+'"></script>');//photo
