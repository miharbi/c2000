...
function foco(){
  ...
  if(isset(idobj)){
	...
	for(i=0;i<inputs.length;i++){ 
			...
			inputs[i].style.backgroundColor = '#696969';
		...
		if(inputs[i].type!='hidden'){
		   ...
		   if(destino.type!='button'&&destino.type!='submit'&&destino!='reset'){
			    ...
			    if(error){
					...
					destino.style.backgroundColor = '#F4F4F4';
		 ...
		 for (i=0; i<id.length; i++){
             ...
             if (pos!=-1){
		         ...
		         if ((ext!='png' && ext!='PNG') && (ext!='jpg' && ext!='JPG') && (ext!='gif' && ext!='GIF') && (ext!='jpeg' && ext!='JPEG')){
		            alert("<strong>Tipo de im&aacute;gen incorrecto</strong><br><br>El nombre del archivo contiene puntos o el tipo de archivo es incorrecto.<br><br><strong>Archivos permitidos:</strong><br>\"JPG\" o \"GIF\" o \"JPEG\"\n");
		             return false;
				 }
	         }else{
				alert('<strong>'+msj[i]+"</strong> es requerido(a)!");
				 return false;
			 }
		 }
		 return true;
...
function validaFileText2(ids, nombres){
		 ...
		 for (i=0; i<id.length; i++){
             ...
             if (pos!=-1){
		         var ext = document.getElementById(id[i]).value.substr(pos+1,3);
		         if ((ext!='zip' && ext!='ZIP')){
		            alert("<strong>Tipo de archivo incorrecto</strong><br><br>El nombre del archivo contiene puntos o el tipo de archivo es incorrecto.<br><br><strong>Archivos permitidos:</strong><br>\"ZIP\"\n");
		             return false;
				 }
	         }else{
				alert('<strong>'+msj[i]+"</strong> es requerido(a)!");
...
function validaFileText3(ids, nombres){
		 ...
		 for (i=0; i<id.length; i++){
             ...
             if (pos!=-1){
		         var ext = document.getElementById(id[i]).value.substr(pos+1,3);
		         if ((ext!='xml' && ext!='XML')){
		            alert("<strong>Tipo de archivo incorrecto</strong><br><br>El nombre del archivo contiene puntos o el tipo de archivo es incorrecto.<br><br><strong>Archivos permitidos:</strong><br>\"XML\"\n");
		             return false;
				 }
	         }else{
				alert('<strong>'+msj[i]+"</strong> es requerido(a)!");
...
function valida(){ // requerido=" label", opcional(tamanio="tamaño")    
	...
	for (i=0; i<inputs.length; i++){
         ...
         if (inputs[i].getAttribute('tipo') && !validateForm(inputs[i].id) && inputs[i].value!=""){
			// msj+='Y el valor debe tener el formato: ['+ Mayusculas(inputs[i].getAttribute('tipo')) +'].';
			msj+='Y el valor debe tener el formato: 1/2/2011.';
			 paso = true;
		 }
...
function validateForm(id){
	//alert(id);
		 ...
		 switch (tipo){
				 ...
				 case 'fecha' : regex = /^\d{1,2}\/\d{1,2}\/\d{4}$/;                       break; //  01/01/2009	 
...
function searchBox(titulo, label) {
	     var band;
		 var error = false;
		prompt('<h1>'+titulo+'</h1>'+label,'' ,{ onComplete: 
...
function confirma(titulo, msj, url){//***********************************************************
         var band = false;
		confirm('<h1>'+titulo+'</h1><p>'+msj+'</p>', {onComplete: 
					 function (returnvalue) { 
							   if (returnvalue)
								   inicio(url);
					 }
		 ...
		 if (valor('band')=='Ok'){
			 for (i=0; i<id.length; i++){				  
				  ...
				  if (document.getElementById('chk_'+id[i]).checked == true && ltrim(valor('txtCant_'+id[i]))==''){
				     alert("<strong>Cantidad</strong> es requerida(a)!");
					  bandera = false;					  
					  paso = true;
					  break;
				  }else{
...
/*function validarFormEventos(){
	 titulo     =document.getElementById('titulo').value;
	 descripcion=document.getElementById('descripcion').value;
	 lugar      =document.getElementById('lugar').value;
	 if( (validateForm("fecha_ini")) && (validateForm("fecha_fin")) && (titulo!='') && (descripcion!='') && (lugar!='') ){
		return true;
	 }else{
		alert('Todos los campos son requeridos y las fechas deben tener el formato correcto ');
	 }
...
//Efecto de desaparecer el TR de id=homeInstalaciones	
	...
	//$(".navegador").click(function(){
			...
			$('#homeInstalaciones').remove() ;	// Se oculta  hide()
