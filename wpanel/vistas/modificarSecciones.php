...
<?php  foreach ($total as $value) {
     ...
     if ($value['padre'] != 0 && $value['padre'] != 55) {
         $hijo = '*&nbsp;&nbsp;&nbsp;&nbsp;'; /*$color='#000';*/
     } else {
         $hijo = '';
     }
...
<?php 
     ...
     print"$hijo <b>$value[titulo]</b>";
     if ($value['tipo'] == '5') {
         echo '&nbsp;&nbsp;-----Div----';
     }
     if ($hijo == '' && $value['fijo'] == 'f' && $value['padre'] != 55 && $value['home'] == '0') {
         ?>&nbsp;&nbsp;<input name="" type="button" value="Crear División" onclick="redirect('controladores/insertSecciones.php?nombre='+prompt('Introduzca nombre de la División')+'&enlace=divisor&idpadre=<?=$value['id']?>');" /><?php 
     }
     ?>
	&nbsp;
    </td>
