...
<?php
    ...
    $res=mysql_query("SELECT id,titulo,fijo,contenido, home FROM `contenidos` WHERE tipo=1 AND (padre=0 OR padre=55) AND status=1 ORDER BY home DESC, orden ASC")or die(mysql_error());
	  ...
	  while($row = mysql_fetch_assoc($res)){
		 if($row['fijo']=='v'){$enlace=$row['contenido'];}else{$enlace=$row['id'];}  // Si es un enlace fijo o no
		 if($row['id']==$_GET[id])$_PADRE=true;
	     $menu.='<tr><td  width="210" class="menu_izq2" >&nbsp;<img src="img/flecha.png" height="10" width="7">&nbsp;&nbsp;<a href="?id='.$enlace.'">'.($row['home']==1?'Inicio':$row['titulo']).'</a></td></tr>';	  
