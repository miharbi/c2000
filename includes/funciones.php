...
function quitar_inyect()
{
    ...
    $filtro = ['"','\\',"'",'|','{','}','[',']','*','>', '<', 'INSERT ' , 'insert ', 'UPDATE', 'update', 'DELETE', 'delete',' x00 ','\\', '\\\\', ' x1a '];
...
function validar_tipo($archivo, $tipos_validos = ['.png', '.jpg', '.gif'])
{
    $extencion = strrchr($archivo, '.');

    foreach ($tipos_validos as $tipo) {
        if (strtolower($tipo) == strtolower($extencion)) {
            return $tipo;
        }
    }
...
{
    $entra = [ultimasNoticias(),ultimosContenidos()];
    $sale = ['{ultimasNoticias}','{ultimosContenidos}'];
